...
jQuery( document ).ready(function( $ ) {
	...
	// Event handler to edit a site when the di-site-submit_edit button is clicked. This function depends on GET parameters set up by WordPress and defined in class-ubc-di-wp-list-table-site.php.
	jQuery( '#di-site-submit_edit' ).click(function() {
		editSiteSubmit();
	});

	// Event handler to delete a site. This function depends on GET parameters set up by WordPress and defined in class-ubc-di-wp-list-table-site.php.
	...
	// Placed here to correct Google Maps bug on initial load
...
/**
 * AJAX call to class-ubc-di-admin-site.php's ubc_di_site_editer_callback(), inserting
 * a ubc_di_site post and updating the window
 */
function addSite() {
	var data = {
		'action': 'site_adder',
		'ubc_di_nonce_field': escapeHTML( jQuery( '#di-nonce-field' ).val() ),
		'ubc_di_site_title': escapeHTML( jQuery( '#di-site-title' ).val() ),
		'ubc_di_site_description': escapeHTML( jQuery( '#di-site-description' ).val() ),
		'ubc_di_site_latitude': escapeHTML( jQuery( '#di-site-latitude' ).val() ),
		'ubc_di_site_longitude': escapeHTML( jQuery( '#di-site-longitude' ).val() )
	};
...
function editSiteSubmit() {
	var data = {
		'action': 'site_editer',
		'ubc_di_nonce_field': escapeHTML( jQuery( '#di-nonce-field' ).val() ),
		'ubc_di_site_id': escapeHTML( jQuery( '#di-site-id_edit' ).attr('value') ),
		'ubc_di_site_title': escapeHTML( jQuery( '#di-site-title_edit' ).val() ),
		'ubc_di_site_description': escapeHTML( jQuery( '#di-site-description_edit' ).val() ),
		'ubc_di_site_latitude': escapeHTML( jQuery( '#di-site-latitude_edit' ).val() ),
		'ubc_di_site_longitude': escapeHTML( jQuery( '#di-site-longitude_edit' ).val() )
	};
	jQuery.post( ajax_object.ajax_url, data, function( response ) {
		location.reload();
	});
